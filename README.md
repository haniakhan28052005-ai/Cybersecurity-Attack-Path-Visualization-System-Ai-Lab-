...
An Artificial Intelligence and Cybersecurity project that demonstrates how graph search algorithms can be used to simulate cyber-attack propagation, intelligent traversal, and optimal path detection within a computer network.

This project models a cybersecurity infrastructure as a weighted graph where:

* **Nodes** represent systems and devices.
...
In modern cybersecurity environments, attackers often move through interconnected systems to reach sensitive resources. Understanding these attack paths is essential for building secure and resilient networks.
...
```text
Attacker → Firewall → Mail Server → Web Server
→ Admin PC → Backup Server → Database Server → Cloud Node
```
...
```text
Discovered Path:
Attacker → Firewall → Admin PC → Database Server
...
```text
Optimal Path:
Attacker → Firewall → Web Server → Database Server
